...
describe('VeryLongPool', () => {
      ...
      describe('success cases', () => {
        ...
        describe('above current price', () => {
          it('transfers token0 only', async () => {
            await expect(mint(wallet.address, -22950, 0, 10000))
              .to.emit(token0, 'Transfer')
              .withArgs(wallet.address, pool.address, 21502)
              .to.not.emit(token1, 'Transfer')
            expect(await token0.balanceOf(pool.address)).to.eq(9996 + 21502)
            expect(await token1.balanceOf(pool.address)).to.eq(1000)
          })

          it('max tick with max leverage', async () => {
            await mint(wallet.address, maxTick - tickSpacing, maxTick, BigNumber.from(2).pow(102))
            expect(await token0.balanceOf(pool.address)).to.eq(9996 + 688803125)
            expect(await token1.balanceOf(pool.address)).to.eq(1000)
          })

          it('works for max tick', async () => {
            await expect(mint(wallet.address, -23000, maxTick, 10000))
              .to.emit(token0, 'Transfer')
              .withArgs(wallet.address, pool.address, 31581)
            expect(await token0.balanceOf(pool.address)).to.eq(9996 + 31581)
            expect(await token1.balanceOf(pool.address)).to.eq(1000)
          })

          it('removing works', async () => {
            await mint(wallet.address, -250, 0, 10000)
            await pool.burn(-250, 0, 10000)
            const { amount0, amount1 } = await pool.callStatic.collect(wallet.address, -250, 0, MaxUint128, MaxUint128)
            expect(amount0, 'amount0').to.eq(125)
            expect(amount1, 'amount1').to.eq(0)
          })

          it('adds liquidity to liquidityGross', async () => {
            await mint(wallet.address, -250, 0, 100)
            expect((await pool.ticks(-250)).liquidityGross).to.eq(100)
            ...
            await mint(wallet.address, -250, tickSpacing, 150)
            expect((await pool.ticks(-250)).liquidityGross).to.eq(250)
            expect((await pool.ticks(0)).liquidityGross).to.eq(100)
          ...
          it('removes liquidity from liquidityGross', async () => {
            await mint(wallet.address, -250, 0, 100)
            await mint(wallet.address, -250, 0, 40)
            await pool.burn(-250, 0, 90)
            expect((await pool.ticks(-250)).liquidityGross).to.eq(50)
            expect((await pool.ticks(0)).liquidityGross).to.eq(50)
          })

          it('clears tick lower if last position is removed', async () => {
            await mint(wallet.address, -250, 0, 100)
            await pool.burn(-250, 0, 100)
            const { liquidityGross, feeGrowthOutside0X128, feeGrowthOutside1X128 } = await pool.ticks(-250)
          ...
          it('only clears the tick that is not used at all', async () => {
            ...
            let { liquidityGross, feeGrowthOutside0X128, feeGrowthOutside1X128 } = await pool.ticks(-250)
            expect(liquidityGross).to.eq(0)
            expect(feeGrowthOutside0X128).to.eq(0)
            expect(feeGrowthOutside1X128).to.eq(0)
        ...
        describe('below current price', () => {
          it('transfers token1 only', async () => {
            await expect(mint(wallet.address, -46100, -23050, 10000))
              .to.emit(token1, 'Transfer')
              .withArgs(wallet.address, pool.address, 2161)
              .to.not.emit(token0, 'Transfer')
            expect(await token0.balanceOf(pool.address)).to.eq(9996)
            expect(await token1.balanceOf(pool.address)).to.eq(1000 + 2161)
          })

          it('min tick with max leverage', async () => {
            await mint(wallet.address, minTick, minTick + tickSpacing, BigNumber.from(2).pow(102))
            expect(await token0.balanceOf(pool.address)).to.eq(9996)
            expect(await token1.balanceOf(pool.address)).to.eq(1000 + 688803136)
          })

          it('works for min tick', async () => {
            await expect(mint(wallet.address, minTick, -23050, 10000))
              .to.emit(token1, 'Transfer')
              .withArgs(wallet.address, pool.address, 3159)
            expect(await token0.balanceOf(pool.address)).to.eq(9996)
            expect(await token1.balanceOf(pool.address)).to.eq(1000 + 3159)
          })

          it('removing works', async () => {
            await mint(wallet.address, -46100, -46050, 10000)
            await pool.burn(-46100, -46050, 10000)
            const { amount0, amount1 } = await pool.callStatic.collect(
              wallet.address,
              -46100,
              -46050,
            ...
            expect(amount1, 'amount1').to.eq(2)
          })
          ...
          it('does not write an observation', async () => {
            ...
            await pool.advanceTime(1)
            await mint(wallet.address, -46100, -23050, 100)
            checkObservationEquals(await pool.observations(0), {
              tickCumulative: 0,
              blockTimestamp: TEST_POOL_START_TIME,
              initialized: true,
  ...
  describe('#observe', () => {
    ...
    it('current tick accumulator after two swaps', async () => {
      ...
      expect((await pool.slot0()).tick).to.eq(-4454)
      await pool.advanceTime(4)
      ...
      // -4454*4 + -1558*6
      expect(tickCumulative).to.eq(-27164)
    })
  })
  ...
  describe('limit orders', () => {
    ...
    it('limit selling 0 for 1 at tick 0 thru 1', async () => {
      await expect(mint(wallet.address, 0, 100, expandTo18Decimals(1)))
        .to.emit(token0, 'Transfer')
        .withArgs(wallet.address, pool.address, '4987272070749097')
      // somebody takes the limit order
      await swapExact1For0(expandTo18Decimals(2), other.address)
      await expect(pool.burn(0, 100, expandTo18Decimals(1)))
        .to.emit(pool, 'Burn')
        .withArgs(wallet.address, 0, 100, expandTo18Decimals(1), 0, '5012269623051203')
        .to.not.emit(token0, 'Transfer')
        .to.not.emit(token1, 'Transfer')
    ...
    it('limit selling 1 for 0 at tick 0 thru -1', async () => {
      await expect(mint(wallet.address, -100, 0, expandTo18Decimals(1)))
        .to.emit(token1, 'Transfer')
        .withArgs(wallet.address, pool.address, '4987272070749097')
      // somebody takes the limit order
      await swapExact0For1(expandTo18Decimals(2), other.address)
      await expect(pool.burn(-100, 0, expandTo18Decimals(1)))
        .to.emit(pool, 'Burn')
        .withArgs(wallet.address, -100, 0, expandTo18Decimals(1), '5012269623051203', 0)
        .to.not.emit(token0, 'Transfer')
        .to.not.emit(token1, 'Transfer')
    ...
    describe('fee is on', () => {
      beforeEach(() => pool.setFeeProtocol(2000, 2000))
      it('limit selling 0 for 1 at tick 0 thru 1', async () => {
        await expect(mint(wallet.address, 0, 100, expandTo18Decimals(1)))
          .to.emit(token0, 'Transfer')
          .withArgs(wallet.address, pool.address, '4987272070749097')
  ...
  describe('#feeProtocol', () => {
    ...
    it.skip('is initially set to 0', async () => {
      expect((await pool.slot0()).feeProtocol).to.eq(0)
    })

    it('can be changed by the owner', async () => {
      await pool.setFeeProtocol(2000, 2000)
      expect((await pool.slot0()).feeProtocol).to.eq(131074000)
    })
    ...
    it('position owner gets partial fees when protocol fee is on', async () => {
      ...
      expect(token0Fees).to.be.eq('399999999999999')
    ...
    describe('#collectProtocol', () => {
      ...
      it('fees collected can differ between token0 and token1', async () => {
        await pool.setFeeProtocol(2000, 3000)
        ...
        await expect(pool.collectProtocol(other.address, MaxUint128, MaxUint128))
          ...
          .withArgs(pool.address, other.address, '149999999999999')
      })
    })
    ...
    it('fees collected by lp after two swaps with intermediate withdrawal', async () => {
      ...
      expect(token0Fees).to.eq('399999999999999')
      expect(token1Fees).to.eq(0)
      ...
      describe('post initialize', () => {
        ...
        it('swapping across gaps works in 1 for 0 direction', async () => {
          ...
          await expect(pool.burn(120000, 121200, liquidityAmount))
            .to.emit(pool, 'Burn')
            .withArgs(wallet.address, 120000, 121200, liquidityAmount, '30024444262341', '997499999999999999')
            .to.not.emit(token0, 'Transfer')
            .to.not.emit(token1, 'Transfer')
        ...
        it('swapping across gaps works in 0 for 1 direction', async () => {
          ...
          await expect(pool.burn(-121200, -120000, liquidityAmount))
            .to.emit(pool, 'Burn')
            .withArgs(wallet.address, -121200, -120000, liquidityAmount, '997499999999999999', '30024444262341')
            .to.not.emit(token0, 'Transfer')
            .to.not.emit(token1, 'Transfer')
    ...
    describe('after liquidity added', () => {
      ...
      describe('fee off', () => {
        it('emits an event', async () => {
          await expect(flash(1001, 2001, other.address))
            .to.emit(pool, 'Flash')
            .withArgs(swapTarget.address, other.address, 1001, 2001, 3, 6)
        })
        ...
        it('calls the flash callback on the sender with correct fee amounts', async () => {
          await expect(flash(1001, 2002, other.address)).to.emit(swapTarget, 'FlashCallback').withArgs(3, 6)
        })
      ...
      describe('fee on', () => {
        ...
        it('increases the fee growth by the expected amount', async () => {
          ...
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            BigNumber.from(5).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            BigNumber.from(9).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
        })
        it('allows donating token0', async () => {
          await expect(flash(0, 0, constants.AddressZero, 567, 0))
          ...
          expect(token0ProtocolFees).to.eq(113)

          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            BigNumber.from(454).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
        })
        it('allows donating token1', async () => {
          await expect(flash(0, 0, constants.AddressZero, 0, 678))
          ...
          expect(token1ProtocolFees).to.eq(135)

          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            BigNumber.from(543).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
        })
        it('allows donating token0 and token1 together', async () => {
          await expect(flash(0, 0, constants.AddressZero, 789, 1234))
          ...
          const { token0: token0ProtocolFees, token1: token1ProtocolFees } = await pool.protocolFees()
          expect(token0ProtocolFees).to.eq(157)
          expect(token1ProtocolFees).to.eq(246)

          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            BigNumber.from(632).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            BigNumber.from(988).mul(BigNumber.from(2).pow(128)).div(expandTo18Decimals(2))
          )
        })
      })
    })
